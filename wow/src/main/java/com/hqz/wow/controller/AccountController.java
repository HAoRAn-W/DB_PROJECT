...
public class AccountController {
    ...
    public String registerSaveCorp(@ModelAttribute("corpCustomerVO") CorpCustomerVO corpCustomerVO, Model model) {
        //todo parameter check, employeeId unique check
        if(customerService.checkIfCustomerExist(corpCustomerVO.getCustomer().getEmail())) {
        ...
        try {
            ...
            String password = bCryptPasswordEncoder.encode(corpCustomerVO.getCustomer().getPassword());
            corpCustomerVO.getCustomer().setPassword(password);
            // 写入数据库
